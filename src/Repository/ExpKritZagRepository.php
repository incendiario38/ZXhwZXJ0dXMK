...
class ExpKritZagRepository extends ServiceEntityRepository
{
    ...
    public function getStd(int $std, int $mkb): array
    {
...
where t.phase = (select phase from exp_std where id_std = :std)
  ...
  and (upper(t.mkb) like upper('%'||(select kod_mkb from exp_std_mkb where id = :mkb)||'%') or upper(t.mkb) like upper('%'||(select SUBSTRING(kod_mkb, 0, POSITION('.' in kod_mkb)) from exp_std_mkb where id = :mkb)||'%') )
SQL;

        return $this->getEntityManager()->createNativeQuery($sql, $rsm)->setParameters(['std' => $std,
            'mkb' => $mkb,])->getResult();
    }
}
